...
public class EvaluateBooleanExpressionToTrue {
    //MEMOIZATION
    //TC:~O(N^3) SC:O(2*N^2)+O(N)

    static final int MOD = 1000000007;
    public static long evaluateExpression1(String exp, int i, int j, int isTrue, Long[][][] dp){
        ...
        for (int ind = i + 1; ind <= j - 1; ind += 2) {
            long lT = evaluateExpression1(exp, i, ind - 1, 1, dp);
            long lF = evaluateExpression1(exp, i, ind - 1, 0, dp);
            long rT = evaluateExpression1(exp, ind + 1, j, 1, dp);
            long rF = evaluateExpression1(exp, ind + 1, j, 0, dp);
    ...
    static int evaluateExpWays1(String exp) {
        int n = exp.length();
        ...
        return (int) evaluateExpression1(exp, 0, n - 1, 1, dp);
    }


    //TABULATION
    //TC:O(N^3) SC:O(2*N^2)
    public static long evaluateExpression2(String exp){
        int n = exp.length();
    ...
    public static void main(String[] args) {
        String exp = "F|T^F";
        int ways = evaluateExpWays1(exp);
        System.out.println(ways);
        long ways2 = evaluateExpression2(exp);
